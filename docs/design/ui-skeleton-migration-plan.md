...
| 2026-09-08 | ✅ **UI 层收尾包（v0.0.39 搭车项）：`AppPage.title` 可空 + 共享组件收敛 + 文案 l10n/动态化** | 34 页迁移收官后的小批量清理：① 引擎 `AppPage.title` 由 `String` 改为 `String?`（纯增量，未传且不传 `titleWidget` 时渲染 null 标题），`test/app_page_test.dart` 补第 15/16 用例；② `display_settings_page` 私有 `_iosSwitchRow`（21 处）全部收敛到共享 `AppSwitchRow`，删私有 helper、去 `ios_switch` import；③ `settings_page`（批次 0 样板页）`ThemeMode` 弹层由裸 `showModalBottomSheet` 改为 `showAppSheet`+`AppSheet`，4 处私有 `_iosSectionCard` 收敛到 `AppSectionCard`（配色 white10/white0.96 与共享版逐字相同，零视觉漂移），去冗余 `card_surface` import；④ 文案修复：`more_page` 硬编码「LLM排行榜」走 l10n（`app_*.arb` 加 `morePageLlmLeaderboard` + `flutter gen-l10n`），`terminal_placeholder` 硬编码版本号改为 `package_info_plus` 动态读取。⚠️ `theme_settings` 的 `_iosSwitchRow` **有意保留私有**（需 subtitle 且横内边距 `AppGap.md`；共享 `AppSwitchRow` 不支持 subtitle、内边距固定 `AppGap.sm`），属经验 #43「不等价的私有实现应保留」——故待办 E 仅落地 `backup` + `display_settings` 两页。`flutter analyze` **3062 → 3061（净 −1，零新增问题）**；`app_page_test` **16/16**、全量测试 21 过 / 1 败（`widget_test.dart` 模板，同基线）。 |
